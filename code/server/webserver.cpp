...
{
    // /home/panan/WebServer-master/
    srcDir_ = getcwd(nullptr, 256); // 获取当前的工作路径
    assert(srcDir_);
    // /home/panan/WebServer-master/resources/
...
{
    // 设置监听文件描述符状态EPOLLRDHUP 检测能否正常关闭
    listenEvent_ = EPOLLRDHUP;
    // 设置通信文件描述符状态EPOLLONESHOT 一个socket在关闭前只能够被触发一次
    // 这样可以保证一个socket只被一个线程处理
    connEvent_ = EPOLLONESHOT | EPOLLRDHUP;
    switch (trigMode) {
        case 0:
            break;
        case 1:
            // epoll的ET模式 只通知一次，需要非阻塞、一次性读完数据
            connEvent_ |= EPOLLET;
            break;
        case 2:
            // 设置监听文件描述符listenEvent_为ET模式 默认为LT模式
    ...
    HttpConn::isET = (connEvent_ & EPOLLET); // 判断通信文件描述符connEvent_是否为ET模式
}
...
void WebServer::Start()
{
    ...
    // 主线程不断调用 epoller_->Wait(timeMS) 检测是否事件到达
    while (!isClose_) {
...
void WebServer::DealRead_(HttpConn *client)
{
    ...
    // 将WebServer::OnRead_函数与client参数绑定,生成一个std::function对象(函数对象),
    // 然后作为一个任务添加到线程池中
    /*
        // 生成一个函数对象
        auto task = std::function<void()> (std::bind(
            &WebServer::OnRead_,     // 函数
            this,                     // 函数对象
            client                    // 参数
        ));
    ...
    */
    // 当线程池中的某个线程执行这个任务时,它会调用task这个函数对象,即执行OnRead_函数,并传入client参数
...
// 业务逻辑的处理
//如果client->process()返回true,则表示客户端的请求处理尚未完成,需要继续监听其EPOLLOUT事件,以继续发送响应;
//如果返回false,则表示客户端的请求已完成处理,需要恢复监听其EPOLLIN事件,等待新请求。
void WebServer::OnProcess(HttpConn *client)
{
    if (client->process()) {
        // 修改通信文件描述符 监听是否可写事件 EPOLLOUT
...
bool WebServer::InitSocket_()
{
    ...
    SetFdNonblock(listenFd_); // 设置监听文件描述符为非阻塞
...
{
    assert(fd > 0);
    // int flag = fcntl(fd, F_GETFD, 0); // 获取文件描述符
